...
class StagHunt(MultiAgentEnv):
    ...
    def render(self, batch=0, grid=None):
        # TODO!
        # pass
        ...
        for i in range(grid.shape[2]):
            grid[:, :, i] *= i + 1
        grid = np.sum(grid, axis=2, dtype=np.int32)
        # render
        screen_width = 10*50
        screen_height = 10*50
        if self.viewer is None:
            ...
            for i in range(len(grid)):
                for j in range(len(grid[0])):
                    hline = rendering.Line((i*50, j*50),(i*50, 10*50))
                    vline = rendering.Line((i*50, j*50), (10*50, j*50))
                    self.viewer.add_geom(hline)
                    self.viewer.add_geom(vline)
        img_dir = os.path.join(os.path.dirname(__file__), 'img')
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                if grid[i][j] == 1:
                    wolve = rendering.Image(fname=os.path.join(img_dir, 'wolve.png'), width=50, height=50, x=i*50, y=j*50)
                    self.viewer.add_onetime(wolve)
                if grid[i][j] == 2:
                    deer = rendering.Image(fname=os.path.join(img_dir, 'deer.png'), width=50, height=50, x=i*50, y=j*50)
                    self.viewer.add_onetime(deer)
        # print('grid', grid)
        self._grid = grid
